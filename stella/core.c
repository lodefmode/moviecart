...
#define WRITE_COLOR(X)    \
    { \
		{ streamByte = *sd_ptr_color++; \
		  streamByte = (streamByte & 0xf0) | shiftBright[(streamByte & 0x0f) + mainBright]; \
		  if (forceColor) \
